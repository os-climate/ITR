...
class DataWarehouse(ABC):
    ...
    def get_preprocessed_company_data(self, company_ids: List[str]) -> List[ICompanyAggregates]:
        ...
        df_company_data.loc[:, self.column_config.CUMULATIVE_TRAJECTORY] = self._get_cumulative_emissions(
            projected_emissions_intensity=self.company_data.get_company_projected_intensities(company_ids),
            projected_production=projected_production).to_numpy()

        df_company_data.loc[:, self.column_config.CUMULATIVE_TARGET] = self._get_cumulative_emissions(
            projected_emissions_intensity=self.company_data.get_company_projected_targets(company_ids),
            projected_production=projected_production).to_numpy()
