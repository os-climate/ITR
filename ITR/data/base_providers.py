    ...
    def _calculate_target_projections(self,
        ...
        for c in self._companies:
            if c.projected_targets is not None:
                ...
                c.projected_targets = EITargetProjector().project_ei_targets(c.target_data, c.historic_data,
                                                                             bm_production_data)
...
class EITargetProjector(object):
    ...
    def project_ei_targets(self, targets: List[ITargetData], historic_data: IHistoricData,
                           production_bm: pd.Series) -> ICompanyEIProjectionsScopes:
        """Input:
            ...
            while scope_targets_intensity or scope_targets_absolute:
                ...
                if target.target_type == "intensity":
                    ...
                    if ei_projection_scopes[scope] is not None:
                        ...
                        last_year_data = IEmissionRealization(year=last_year,
                                                              value=last_year_ei_data.value * last_year_prod)
                    else:
