...
from ITR.data.base_providers import BaseCompanyDataProvider, BaseProviderProductionBenchmark, BaseProviderIntensityBenchmark
from ITR.interfaces import ICompanyData, EScope, ETimeFrames, PortfolioCompany, IEmissionIntensityBenchmarkScopes, IProductionBenchmarkScopes
...
# load company data
with open(company_json) as json_file:
    parsed_json = json.load(json_file)
companies = [ICompanyData.parse_obj(company_data) for company_data in parsed_json]
base_company_data = BaseCompanyDataProvider(companies)
...
OECM_warehouse = DataWarehouse(base_company_data, base_production_bm, OECM_EI_bm)
TPI_warehouse = DataWarehouse(base_company_data, base_production_bm, TPI_EI_bm)
TPI_below_2_warehouse = DataWarehouse(base_company_data, base_production_bm, TPI_below_2_EI_bm)

# dummy_portfolio = "example_portfolio.csv"
dummy_portfolio = "example_portfolio_clean.csv"
...
def update_graph(
    ...
    def f(row):
        if (pd.isna(row['ghg_s1s2']) and row['cumulative_target']==0):
            val = "Not Covered"
        elif (pd.isna(row['ghg_s1s2']) and row['cumulative_target']>0):
            val = "Covered only<Br>by target"
        elif (row['ghg_s1s2']>0 and row['cumulative_target']==0):
    ...
    df['trajectory_score']=df['trajectory_score'].round(decimals = 2) # formating column
    df['target_score']=df['target_score'].round(decimals = 2) # formating column
    ...
    df.rename(columns={'company_name':'Name', 'company_id':'ISIN','region':'Region','sector':'Industry','cumulative_budget':'Emission budget','investment_value':'Notional','trajectory_score':'Historical emissions score', 'target_score':'Target score','temperature_score':'Weighted temperature score'}, inplace=True)
