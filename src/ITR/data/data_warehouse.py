...
class DataWarehouse(ABC):
    ...
    def __init__(
        ...
        if (
            ...
            or benchmarks_projected_ei is not None
        ):
    ...
    def update_benchmarks(
        ...
        if (
            ...
            or self.benchmarks_projected_ei._EI_benchmarks
            != benchmarks_projected_ei._EI_benchmarks
        ):
            ...
            if self.benchmarks_projected_ei is not None and getattr(
                ...
                prev_prod_centric = self.benchmarks_projected_ei._EI_benchmarks[
                    "S1S2"
                ].production_centric
            if getattr(benchmarks_projected_ei._EI_benchmarks, "S1S2", None):
                next_prod_centric = benchmarks_projected_ei._EI_benchmarks[
                    "S1S2"
                ].production_centric
        ...
        if new_ei_bm and (
            new_companies := [
                c for c in self.company_data._companies if "+" in c.company_id
            ]
        ):
            ...
            bm_sectors = (
                bm_ei_df_t.columns.get_level_values("sector").unique().to_list()
            )
            ...
            for c in new_companies:
                ...
                if (sector, c.region) in bm_ei_df_t.columns:
                    ...
                    logger.error(
                        f"No benchmark region data for {orig_id}: sector = {sector}; region = {region}"
                    )
                    continue
            ...
            for orig_id, sectors in sectors_dict.items():
                ...
                sector_ei = [
                    ...
                    if (c.company_id, scope.name)
                    in self.company_data._bm_allocation_index
                ...
                to_allocate_idx = self.company_data._bm_allocation_index[
                    self.company_data._bm_allocation_index.map(
                        lambda x: x[0].startswith(orig_id)
                    )
                ...
                for sector_aligned in aligned_em:
                    ...
                    for scope_tuple in scopes:
                        ...
                        setattr(
                            historic_sector.emissions_intensities, scope.name, ei_list
                        )
            ...
            for c in self.company_data._companies:
                ...
                if c.historic_data:
                    ...
                    if (
                        ...
                        and c.historic_data.emissions_intensities.S3
                    ):
                        _adjust_historic_data(
                            c.historic_data.emissions_intensities,
                            "S1",
                    ...
                    if (
                        c.historic_data.emissions_intensities
                        and c.historic_data.emissions_intensities.S1S2S3
                    ):
                ...
                if c.projected_targets and c.projected_targets.S3:
                    ...
                    def _align_and_sum_projected_targets(targets, primary_scope_attr):
                        ...
                        if isinstance(s3_projections, pd.Series):
                            ...
                            if primary_projections[0].year < s3_projections[0].year:
                                while (
                                    primary_projections[0].year < s3_projections[0].year
                                ):
                    ...
                    try:
                        ...
                        if c.projected_targets.S3:
                            _align_and_sum_projected_targets(
                                c.projected_targets, "S1S2"
                            )
                        else:
                            logger.warning(
        ...
        for c in self.company_data._companies:
            region = c.region
            try:
                bm_company_sector_region = benchmarks_projected_ei._EI_df_t[
                    (c.sector, region)
                ]
            except KeyError:
                try:
                    ...
                    continue
            scopes = benchmarks_projected_ei._EI_df_t[
                (c.sector, region)
            ].columns.tolist()
            if len(scopes) == 1:
    ...
    def estimate_missing_s3_data(self, company: ICompanyData) -> None:
        ...
        if (
            ...
            bm_ei_s3 = self.benchmarks_projected_ei._EI_df_t[
                (sector, region, EScope.S3)
            ]
        if (
            sector,
            region,
            ...
            bm_ei_s1s2 = self.benchmarks_projected_ei._EI_df_t[
                (sector, region, EScope.S1S2)
            ]
        else:
            bm_ei_s1s2 = None
        ...
        if company.projected_intensities.S1S2S3:
            ...
            company.ghg_s1s2 = (
                s1s2_projections[self.company_data.projection_controls.BASE_YEAR]
        ...
        company.ghg_s3 = (
            s3_projections[self.company_data.projection_controls.BASE_YEAR]
            * company.base_year_production
        ).to("t CO2e")
        logger.info(
    ...
    def get_preprocessed_company_data(
        ...
        ).set_index(self.column_config.COMPANY_ID, drop=False)
        ...
        with warnings.catch_warnings():
            ...
            target_year_loc = projected_production.columns.get_loc(
                self.company_data.projection_controls.TARGET_YEAR
            )
