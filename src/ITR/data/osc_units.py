...
def convert_to_annual(x, errors="ignore"):
    ...
    try:
        ...
        if errors != "ignore":
            raise DimensionalityError(
                x, "", extra_msg=f"; dimensionality must contain [time] or 1/[time]"
            )
...
def dimension_as(x, dim_unit):
    ...
    try:
        ...
        raise DimensionalityError(
            x, dim_unit, extra_msg=f"; no compatible dimension not found"
        )
...
class EmissionsQuantity(Quantity):
    ...
    def validate(cls, quantity):
        ...
        if not isinstance(quantity, Quantity):
            raise TypeError(
                f"pint.Quantity required ({quantity}, type = {type(quantity)})"
            )
